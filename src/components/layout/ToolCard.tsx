...
export function ToolCard({ title, description, icon: Icon, category, onClick }: ToolCardProps) {
  return (
    <button
      onClick={onClick}
      className={cn(
        'w-full p-4 sm:p-5 bg-gradient-to-br rounded-xl border text-left transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-blue-500/10 group',
        categoryColors[category] || 'from-blue-500/20 to-indigo-500/20 border-blue-500/30'
      )}
    >
      <div className="flex items-start gap-3 sm:gap-4">
        <div className={cn(
          'w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br flex items-center justify-center flex-shrink-0',
